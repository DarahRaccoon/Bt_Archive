...
\newcommand{\NaturalElem}[4]{
    \ElemLabel{#1}{#2}{\huge {#3}}{#4}
}

\newcommand{\SyntheticElem}[4]{
    \ElemLabel{#1}{#2}{\color{gray}{\huge #3}}{#4}
}

\definecolor{MetalAlcalinoFill}{RGB}{115,115,255}

\begin{tikzpicture}[font=\sffamily]

  % Fill Color Styles
  \tikzstyle{ElementFill} = [fill=yellow!15]
  \tikzstyle{AlkaliMetalFill} = [fill=MetalAlcalinoFill]
  ...
  % Element Styles
  \tikzstyle{Element} = [ElementFill,minimum width=2.5cm, minimum height=2.5cm, node distance=2.75cm]
  ...
  \node[below of=Cs, AlkaliMetal] (Fr) {\NaturalElem{87}{223}{Fr}{XxFranciumxX}};
  ...
  % Actinide
  \node[below of=La, LanthanideActinide, yshift=-0.25cm] (Ac) {\NaturalElem{89}{227}{Ac}{Actinium}};
  ...
  \draw[thick,dotted] (LaLu.south) -- (La.north west)
        (LaLu.south) -- (La.south west);
  % Draw dotted lines connecting Actinide breakout to main table
  \draw[thick,dotted] (AcLr.south) -- (Ac.north west)
        (AcLr.south) -- (Ac.south west);
  ...
  % Legend
   ...
   \fill[AlkalineEarthMetalFill] ($(AlkaliMetal.west) - (1em,2em)$)
     rectangle +(1em, 1em) node[right, yshift=-1.2ex] (AlkalineEarthMetal) {Metal Alcalinoterro};
   ...
   \fill[LanthanideActinideFill] ($(NobleGas.west) - (1em,2em)$)
     rectangle +(1em, 1em) node[right, yshift=-1.2ex] (Lanthanide/Actinide) {Lantanido/Actinido};

  \node at (Ac -| Fr) [draw, Element, fill=white] (legend) {\NaturalElem{Z}{Masa}{\LARGE Símbolo}{Nombre}};
  \node[align=left] at (Ac -| Ra) {black: natural\\\color{gray}gray: man-made};

  % Diagram Title
  \node at (H.west -| Fe.north) [scale=1.75, font={\sffamily\Huge\bfseries}]
