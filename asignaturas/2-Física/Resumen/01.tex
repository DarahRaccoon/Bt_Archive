


\section{Dimensiones y unidades}



\section{Magnitudes fundamentales y derivadas}

    \input{tables/tab:magnitudes_fundamentales}
    \input{tables/tab:magnitudes_derivadas}


\section{Sistemas de unidades}

El objetivo de un sistema de unidades es el de definir en términos de una unidad estandarizada la magnitud de una cantidad medida. En 1960, el Comité Internacional de Pesos y Medidas proporciona definiciones claras respecto a unidades estándar, ayudando a los científicos para que comuniquen sus medidas de manera más precisa. El \acrfull{si} es el más adoptado en el mundo.

\section{Unidades básicas y prácticas}

Unidades básicas o patrón:
\begin{itemize}
    \item Masa (\unit{kg})
    \item Longitud (\unit{m})
    \item Tiempo (\unit{s}).
\end{itemize}

Unidades prácticas:
\begin{itemize}
    \item Masa ($g$)
    \item Longitud ($km$)
    \item Tiempo ($h$).
\end{itemize}


\section{Principio de incertidumbre y cifras significativas}




\section{Notación científica}

La notación científica, también llamada notación exponencial, es una forma de escribir números basada en potencias de 10. Esta se representa con la forma $m \times 10^{n}$, donde:

\begin{itemize}
    \item $m$, llamada mantisa, es un número decimal cuyo entero es una sola cifra distinta de 0
    \item $10^{n}$ es la potencia u orden de magnitud, y es un exponente entero que indica 
\end{itemize}

Esta forma de representar números surgida en el siglo \rom{3} a.C. y descrita por Arquímedes en su obra El Contador de Arena, es muy conveniente para escribir números muy grandes o pequeños y hacer cálculos con ellos, lo cual es muy útil en materias donde se realizan mediciones muy grandes o muy pequeñas, como astronomía o microbiología.

Algunos ejemplos de notación científica son los siguientes:

\begin{enumerate}
    \item Masa del protón: $1.67 \times 10^{-27}$
    \item Masa del electrón: $9.11 \times 10^{-31}$
    \item Número de Avogadro: $6.02 \times 10^{23}$
    \item 1 año luz: $1 \times 10^{16}m$
    \item Diámetro del leucocito: $6.5 \times 10^{-3}cm$
\end{enumerate}